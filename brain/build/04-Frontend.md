...
- **The split model**: V8-isolate **edge for the request path** (caching, streaming, geo) and **Node.js for everything stateful or heavy**. Headline: **Next.js 16.3 deprecates `runtime='edge'`** (runs with build warnings; Cloudflare Workers remains the edge-native platform) — routes run on Node, and `middleware.ts` became `proxy.ts`, which runs on the Node.js runtime.
...
- **2026 set: LCP < 2.5s, INP < 200ms (replaced FID), CLS < 0.1.** Only ~48% of mobile origins pass all three (2025 Web Almanac); LCP is the most-failed metric on mobile.
...
- **It's law, with deadlines** ⚠️ verified: 2026-08: EU **EAA** (e-commerce, banking, transport) enforceable since **Jun 28 2025** for new products, legacy → 2030; EN 301 549 v4.1.1 → WCAG 2.2 published Sept 2026; OJEU citation (legal effect) targeted ~30 Nov 2026. US ADA **Title II** (public entities): **Apr 26 2027** (pop ≥50k) / Apr 2028 (smaller). Title III (private): WCAG 2.1 AA is de facto — **~8,667 ADA Title III federal filings in 2025 (−2%; 3,117 website-accessibility-specific, +27%)**, settlements $5k–75k (Fashion Nova $5.15M). CO HB21-1110 live ($3,500/violation).
...
- **Overlays are dead**: FTC fined accessiBe **$1M** (2025); ~22.6% of H1-2025 ADA suits targeted sites *with* overlays (UsableNet estimate — verify before relying); courts reject overlay defenses. Never install one.
...
- **Design tokens**: the W3C DTCG spec (2025.10) is a **Community Group Final Report** — not on the W3C Standards Track — `$value`/`$type`/`$description`; backed by Figma (native since late 2025), Tokens Studio, Style Dictionary (v5 adopts it). Runtime layer = CSS custom properties; theming via `data-theme` attribute swapping `$value` (no JS provider). Token adoption ~84% of product teams.
...
- **AI Overviews 2026**: trigger on ~25.8% of all US searches and ~7.9% of pure local searches (late 2025); the ~68% figure is Whitespark's 2025 local-business-query study — cite accordingly; AI packs show ~68% fewer businesses → **thin profiles get excluded**; cited businesses see ~35% higher CTR. Services/pricing/FAQ content is AI citation fuel — but one page can't win multiple city+service keywords; add dedicated service/FAQ pages only if chasing them.
...
- **Expo/React Native 2026**: **SDK 57** (RN 0.86); **New Architecture mandatory** (SDK 55+; legacy frozen Jun 2025); edge-to-edge mandatory on Android 15+; **Expo Go remains on both stores** (App Store build frozen at SDK 54; SDK 55+ never approved) → Expo now positions it as educational; use `expo-dev-client` dev builds + EAS Build/Submit/Update (OTA, Hermes bytecode diffing); Expo Router v56 file-based, decoupled from React Navigation, adds web SSR. Not enough → `expo prebuild` (CNG) + config plugins; custom native via Expo Modules API.
