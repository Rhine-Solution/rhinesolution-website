...
### 2026-09-07 — Cloning a website fully: SPA shells, corrupted bundles, Nuxt payload paths, and the CMS/CORS trick
- **What happened**: Fully cloned hubtown.co.in (Nuxt SPA + Three.js 3D scenes + Sanity CMS) onto the PC — 259 files / 52 MB, including the scroll-driven 3D background. goclone got only the 4-file shell; httrack got the full mirror. Three breakages had to be fixed before it ran: (1) httrack appended 208 bytes to the 2.4MB main JS bundle → `SyntaxError` → re-downloaded the bundle; (2) the embedded Nuxt `__NUXT_DATA__` path was `index.html` so the app fetched `/index.html/_payload.json` → patched the path to `/` + copied `_payload0c88.json` → `_payload.json` per route; (3) Sanity CMS rejects `localhost` origins (CORS) → served the mirror under the real hostname (hosts + self-signed cert + Node HTTPS on 443) so the browser origin matches and the API allows it. Also downloaded every missing 3D asset (8 GLB, 19 KTX2, 2 EXR, 3 point-cloud JSON, Draco + Basis decoders) and branded the clone (R&S logo swap, Login removal).
- **Root cause**: mirror tools only download files, not API/CMS data; SPA content is JS-rendered from APIs; CORS blocks localhost; httrack output is not always byte-clean.
- **Lesson**: a "full website clone" of a modern site needs (a) a deep crawler (httrack), (b) the bundle's asset URLs extracted and downloaded, (c) the Draco/Basis decoders for compressed 3D, (d) the Nuxt payload path fixed, and (e) the CMS/CORS workaround (hosts + self-signed cert + port-443 server under the real domain) — after which the clone is pixel-identical. Never remove a site's intro loader (it gates scrolling). Patch minified error messages (`new Error("Texture not loaded "+e)`) to reveal failing asset URLs. `Ctrl+F5` after every asset fix.
- **Prevention**: full process in , tools in , real walkthrough in .
## Links
...
## 2026-09-08 - Netherlands map rebuild: SSR district-row ids must match rebuilt ev keys; terrain GLB draw mode
- **What happened**: Task 5's hydration gate (cdp-sweep/cdp-load) caught a hard regression from the NL map rebuild: the bundle's ev was rebuilt with NL district keys (west/south/central/north), but the SSR HTML's district rows still carried hubtown's Mumbai ids (district-item-central-suburbs, south-mumbai, western-suburbs, thane). uildMapDistricts does querySelector('#district-item-<key>'), got null, and the three.js CSS2DObject constructor (o_e/exe) threw Cannot read properties of null (reading 'style') on every page.
- **Root cause**: (1) the ev key rename (Task 4) was not reconciled with the SSR district-row ids; (2) a second masked bug surfaced once the first crash was fixed - the regenerated terrain GLB wrote its primitive with glTF draw mode 0 (POINTS) instead of 4 (TRIANGLES), so three.js loaded it as Points not Mesh, ssets.models.terrain never registered, and 
xe.update() threw Cannot read properties of undefined (reading 'x') every frame.
- **Fix**: chrome pair-table in _build/mappings.js renames the 4 ids (central-suburbs->central, south-mumbai->south, western-suburbs->west, thane->north - thane maps to north per the shader's uniform channels); _build/nl-terrain.js draws the terrain primitive with mode 4. District rows are part of the *global* layout (they exist on every page, not just home/projects), so the chrome-table approach (applies to all EN pages; locales regenerate from EN via build-locales) was the right scope.
- **Lesson**: (1) when a build-time data contract changes keys (e.g. district ids, shader uniforms), grep the SSR HTML for consumers of those ids - don't assume which pages carry them; (2) a regression gate that reports one crash can be hiding a second crash behind it - re-run the full sweep after the first fix; (3) glTF primitive draw mode matters: 0=POINTS, 1=LINES, 4=TRIANGLES - a "triangles" mesh written with mode 0 silently loads as Points in three.js and breaks Mesh-only logic (no console error).
- **Prevention**: _build/mappings.js (chrome district-id block), _build/nl-terrain.js (mode 4), _build/verify-nl-map.js + cdp-sweep/cdp-load gates. See [[05-Databases]], [[04-Frontend]].
